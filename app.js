...
async function askGemini(prompt){const model=D.admin?.ai?.model||'gemini-3.8-flash';const fullPrompt=`You are the Classwork Planner study assistant. Answer clearly and accurately. You may use the student's records below. Do not invent records.\n\nSTUDENT DATA:\n${aiContext()}\n\nUSER QUESTION:\n${prompt}`;try{return await GeminiAPI.generate(fullPrompt,model)}catch(e){console.error('Gemini AI error:',e);const msg=String(e?.message||e||'Gemini request failed');if(/app.?check|permission-denied|403/i.test(msg))toast('Gemini is blocked until Firebase AI Logic/App Check is configured.');else if(/not found|404|model/i.test(msg))toast('The selected Gemini model is unavailable. Choose Gemini 3.8 Flash.');else toast(msg);return ''}}
