...
function freshStudentData(){return {profile:{name:'',email:'',roll:''},teachers:{},records:{},extras:[],changes:[],theme:D.theme||'light',schedule:null,reminders:[],syllabus:{},admin:{},settings:{reminderEnabled:true,oneDayBefore:true,reminderLead:60,lastSyncAt:0},_savedAt:0}}
...
let D={profile:{name:'',email:'',roll:''},teachers:{},records:{},extras:[],changes:[],theme:'light',schedule:null,reminders:[],syllabus:{},admin:{},settings:{reminderEnabled:true,oneDayBefore:true,reminderLead:60,lastSyncAt:0},_savedAt:0};
...
function setTheme(){document.documentElement.dataset.theme=(['light','dark','light3d','dark3d'].includes(D.theme)?D.theme:'light')}
...
async function cloudSignUp(email,password,name='',roll=''){
  try{
    ...
    try{localStorage.setItem(STUDENT_SESSION_HINT,'1')}catch(_){}
    D=freshStudentData();
    // Save the identity details entered during the mandatory first-login gate
    // before rendering Settings. They remain editable later from Student Profile.
    D.profile.email=cloudUser?.email||email;
    D.profile.name=String(name||'').trim();
    D.profile.roll=String(roll||'').trim();
    switchToUserLocal(cloudUser?.uid,cloudUser?.email||email);
    // switchToUserLocal may restore a private local copy; the newly entered
    // signup identity is authoritative for this newly authenticated account.
    D.profile.email=cloudUser?.email||email;
    if(String(name||'').trim())D.profile.name=String(name).trim();
    if(String(roll||'').trim())D.profile.roll=String(roll).trim();
    saveAll();
    if($('settings'))show('settings');
