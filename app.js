...
async function loadAdminCloudData(el){
  try{
    ...
    const note=document.querySelector('#studentsPanel .adminNote'); if(note)note.textContent='Cloud-connected students. Select VIEW RECORDS to inspect saved classwork, homework and syllabus.'; refreshAdminCloudPanels();
...
function cloudAggregate(){
  ...
  students.forEach(st=>{
    ...
    Object.entries(st.syllabus||{}).forEach(([subject,arr])=>{
      (Array.isArray(arr)?arr:[]).forEach(item=>syllabus.push({studentName:profile.name||st.name||'Unnamed student',email:profile.email||st.email||'',roll:profile.roll||'',subject,...item}));
    });
  });
...
function refreshAdminCloudPanels(){
  ...
  const records=a.rows, topics=a.syllabus;
  const cl=$('adminClassworkList');
  const hw=$('adminHomeworkList');
  const sy=$('adminSyllabusList');
  if(cl){
    const render=()=>{const sub=$('adminSubjectFilter')?.value||'';const date=$('adminDateFilter')?.value||'';const f=records.filter(x=>(!sub||x.meta.subject===sub)&&(!date||x.meta.date===date));cl.innerHTML=f.slice().sort((a,b)=>(b.meta.date+b.meta.time).localeCompare(a.meta.date+a.meta.time)).map(x=>`<div class="adminRecord"><b>${esc(x.meta.subject)}</b><span>${esc(x.meta.date)} · ${esc(x.meta.time)} · ${esc(x.studentName)}</span><p><strong>Chapter:</strong> ${esc(x.record.chapter||'—')} · <strong>Topic:</strong> ${esc(x.record.topic||'—')}</p><small>Teacher: ${esc(x.record.teacher||'—')} · ${esc(x.email)}${x.roll?' · Roll '+esc(x.roll):''}</small></div>`).join('')||'<div class="empty glass">No classwork records for the selected filter.</div>'};
    cl.dataset.bound='1';render();
    $('adminSubjectFilter')?.addEventListener('change',render);$('adminDateFilter')?.addEventListener('change',render);
  }
  if(hw){
    const render=()=>{const st=$('adminHomeworkFilter')?.value||'';const date=$('adminHomeworkDate')?.value||'';const f=records.filter(x=>x.record.homework&&x.record.homework.trim()&&(!st||(st==='completed'?!!x.record.completed:!x.record.completed))&&(!date||x.meta.date===date));hw.innerHTML=f.slice().sort((a,b)=>(b.meta.date+b.meta.time).localeCompare(a.meta.date+a.meta.time)).map(x=>`<div class="adminRecord"><b>${esc(x.meta.subject)}</b><span>${esc(x.meta.date)} · ${esc(x.meta.time)} · ${esc(x.studentName)}</span><p>${esc(x.record.homework)}</p><small>${x.record.completed?'✓ Completed':'○ Pending'} · Teacher: ${esc(x.record.teacher||'—')} · ${esc(x.email)}${x.roll?' · Roll '+esc(x.roll):''}</small></div>`).join('')||'<div class="empty glass">No homework records for the selected filter.</div>'};render();$('adminHomeworkFilter')?.addEventListener('change',render);$('adminHomeworkDate')?.addEventListener('change',render);
  }
  if(sy){sy.innerHTML=topics.slice().sort((a,b)=>(b.date||'').localeCompare(a.date||'')).map(x=>`<div class="adminRecord"><b>${esc(x.subject)}</b><span>${esc(x.date||'')} · ${esc(x.studentName)}</span><p><strong>Chapter:</strong> ${esc(x.chapter||'—')} · <strong>Topic:</strong> ${esc(x.topic||'—')}</p><small>Teacher: ${esc(x.teacher||'—')} · ${esc(x.email)}${x.roll?' · Roll '+esc(x.roll):''}</small></div>`).join('')||'<div class="empty glass">No syllabus records.</div>';}
  const stats=document.querySelectorAll('.adminStatsGrid .adminStat strong'); if(stats.length>=4){const hwRows=records.filter(x=>x.record.homework&&x.record.homework.trim());stats[0].textContent=a.students.length;stats[1].textContent=records.length;stats[2].textContent=hwRows.length;stats[3].textContent=topics.length;}
  const analytics=document.querySelector('#analyticsPanel .analyticsBars');if(analytics){const hwRows=records.filter(x=>x.record.homework&&x.record.homework.trim());const vals=[hwRows.length?Math.round(hwRows.filter(x=>x.record.completed).length/hwRows.length*100):0,records.length?Math.round(records.filter(x=>x.record.topic).length/records.length*100):0,records.length?Math.round(records.filter(x=>x.record.chapter).length/records.length*100):0];analytics.querySelectorAll('b').forEach((b,i)=>b.textContent=vals[i]+'%');const note=document.querySelector('#analyticsPanel .adminNote');if(note)note.textContent='Analytics are calculated from all student records currently stored in Firebase.';}
}
function buildAdminExportHtml(title,aggregate){
  const {students,rows,syllabus}=aggregate;
  const byStudent=new Map();students.forEach(st=>byStudent.set(st.id||st.uid||Math.random(),st));
  const studentSections=students.map(st=>{const p=st.profile||{};const rs=rows.filter(x=>x.studentId===(st.id||st.uid));const ss=syllabus.filter(x=>x.studentName===(p.name||st.name||'Unnamed student')&&x.email===(p.email||st.email||''));return `<section class="student"><h2>${esc(p.name||st.name||'Unnamed student')}</h2><p><b>Email:</b> ${esc(p.email||st.email||'')} ${p.roll?' · <b>Roll:</b> '+esc(p.roll):''}</p><h3>Classwork & Homework</h3><table><thead><tr><th>Date</th><th>Day</th><th>Time</th><th>Subject</th><th>Chapter</th><th>Topic</th><th>Teacher</th><th>Homework</th><th>Status</th></tr></thead><tbody>${rs.slice().sort((a,b)=>(a.meta.date+a.meta.time).localeCompare(b.meta.date+b.meta.time)).map(x=>{const day=x.meta.date?new Date(x.meta.date+'T00:00:00').toLocaleDateString('en-IN',{weekday:'long'}):'';return `<tr><td>${esc(x.meta.date)}</td><td>${esc(day)}</td><td>${esc(x.meta.time)}</td><td>${esc(x.meta.subject)}</td><td>${esc(x.record.chapter||'')}</td><td>${esc(x.record.topic||'')}</td><td>${esc(x.record.teacher||'')}</td><td>${esc(x.record.homework||'')}</td><td>${x.record.homework?(x.record.completed?'Completed':'Pending'):''}</td></tr>`}).join('')||'<tr><td colspan="9">No classwork/homework records.</td></tr>'}</tbody></table><h3>Syllabus</h3><table><thead><tr><th>Subject</th><th>Chapter</th><th>Topic</th><th>Date</th><th>Teacher</th></tr></thead><tbody>${ss.map(x=>`<tr><td>${esc(x.subject)}</td><td>${esc(x.chapter||'')}</td><td>${esc(x.topic||'')}</td><td>${esc(x.date||'')}</td><td>${esc(x.teacher||'')}</td></tr>`).join('')||'<tr><td colspan="5">No syllabus records.</td></tr>'}</tbody></table></section>`}).join('');
  const overall=rows.slice().sort((a,b)=>(a.meta.date+a.meta.time).localeCompare(b.meta.date+b.meta.time)).map(x=>{const day=x.meta.date?new Date(x.meta.date+'T00:00:00').toLocaleDateString('en-IN',{weekday:'long'}):'';return `<tr><td>${esc(x.studentName)}</td><td>${esc(x.roll)}</td><td>${esc(x.meta.date)}</td><td>${esc(day)}</td><td>${esc(x.meta.time)}</td><td>${esc(x.meta.subject)}</td><td>${esc(x.record.chapter||'')}</td><td>${esc(x.record.topic||'')}</td><td>${esc(x.record.teacher||'')}</td><td>${esc(x.record.homework||'')}</td><td>${x.record.homework?(x.record.completed?'Completed':'Pending'):''}</td></tr>`}).join('');
  return `<!doctype html><html><head><meta charset="utf-8"><title>${esc(title)}</title><style>body{font-family:Arial,sans-serif;padding:20px;color:#111}h1{font-size:24px}h2{margin-top:28px}h3{margin-top:20px}table{width:100%;border-collapse:collapse;margin:8px 0 20px;font-size:9px}th,td{border:1px solid #777;padding:5px;vertical-align:top}th{background:#eee}.student{page-break-before:always}.student:first-of-type{page-break-before:auto}@media print{body{padding:0}@page{size:A4 landscape;margin:10mm}thead{display:table-header-group}}</style></head><body><h1>${esc(title)}</h1><p><b>Generated:</b> ${esc(new Date().toLocaleString('en-IN'))}</p><h2>Overall Report</h2><p><b>Students:</b> ${students.length} · <b>Classwork/Homework records:</b> ${rows.length} · <b>Syllabus entries:</b> ${syllabus.length}</p><table><thead><tr><th>Student</th><th>Roll</th><th>Date</th><th>Day</th><th>Time</th><th>Subject</th><th>Chapter</th><th>Topic</th><th>Teacher</th><th>Homework</th><th>Status</th></tr></thead><tbody>${overall||'<tr><td colspan="11">No records.</td></tr>'}</tbody></table>${studentSections}</body></html>`;
}
function exportCloudPdf(title='Homework Reminder — Complete Student Data'){const a=cloudAggregate();if(!a.students.length){toast('No cloud student data available.');return}const w=window.open('','_blank');if(!w){toast('Allow pop-ups for PDF export.');return}w.document.open();w.document.write(buildAdminExportHtml(title,a).replace('</body>','<script>window.onload=function(){setTimeout(function(){window.focus();window.print()},400)};<\/script></body>'));w.document.close();}
...
function renderAdmin(el){
  ...
  $('adminExport').onclick=()=>exportCloudPdf('Homework Reminder — Complete Overall & Student-wise Data');
  $('adminExportRecords').onclick=()=>{const a=cloudAggregate();const rows=[['Student','Email','Roll','Subject','Date','Day','Time','Chapter','Topic','Homework','Teacher','Status']];a.rows.forEach(x=>rows.push([x.studentName,x.email,x.roll,x.meta.subject,x.meta.date,x.meta.date?new Date(x.meta.date+'T00:00:00').toLocaleDateString('en-IN',{weekday:'long'}):'',x.meta.time,x.record.chapter||'',x.record.topic||'',x.record.homework||'',x.record.teacher||'',x.record.homework?(x.record.completed?'Completed':'Pending'):'']));const csv=rows.map(row=>row.map(v=>`"${String(v).replaceAll('"','""')}"`).join(',')).join('\n');const link=document.createElement('a');link.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));link.download='homework-reminder-all-cloud-records.csv';link.click();toast('✓ Cloud records CSV exported')};
  $('adminExportPdf').onclick=()=>exportCloudPdf('Homework Reminder — Complete Records PDF');
