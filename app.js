...
function show(page){if(page==='admin'&&!adminSession){return showAdminLogin()}document.querySelectorAll('main>section').forEach(x=>x.remove());const s=document.createElement('section');s.id=page;$('main').appendChild(s);$('pageTitle').textContent={today:'Today',homework:'Homework',syllabus:'Syllabus',ai:'Gemini AI',settings:'Settings',admin:'Admin Dashboard',reminders:'Reminders'}[page]||page;$('dateLine').textContent=page==='today'?fmt(today):'';({today:renderToday,homework:renderHomework,syllabus:renderSyllabus,ai:renderAI,settings:renderSettings,admin:renderAdmin,reminders:renderReminders}[page]||renderToday)(s)}
...
function renderAdmin(el){
  ...
  $('adminLogout').onclick=async()=>{try{await CloudAPI.adminSignOut()}catch(e){} adminUser=null;adminSession=false;show('settings');toast('Admin logged out')};
  $('adminLogout2').onclick=async()=>{try{await CloudAPI.adminSignOut()}catch(e){} adminUser=null;adminSession=false;show('settings');toast('Admin session closed')};
...
CloudAPI.onAuthStateChanged(async user=>{
  ...
  if(user){
    ...
    if(!adminUser&&!adminAuthInProgress) showStartupAuth();
    if($('settings'))show('settings');
  }
});
CloudAPI.onAdminAuthStateChanged(user=>{adminUser=user||null;if(user)closeStartupAuth();if(!user&&adminSession){adminSession=false;show('settings');if(!cloudUser)showStartupAuth()}});
