...
async function exportCloudPdf(title='Homework Reminder — Complete Firebase Cloud Data'){
  if(!await loadFreshCloudStudentsForExport())return;
  const a=cloudAggregate();
  if(!a.students.length){toast('No student data found in Firebase.');return}
  ...
  toast('✓ Firebase cloud data prepared for PDF export');
}
