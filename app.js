...
async function askGemini(prompt){const model=D.admin?.ai?.model||'gemini-3.8-flash';const fullPrompt=`You are the Classwork Planner study assistant. Answer clearly and accurately. You may use the student's records below. Do not invent records.\n\nSTUDENT DATA:\n${aiContext()}\n\nUSER QUESTION:\n${prompt}`;try{return await GeminiAPI.generate(fullPrompt,model)}catch(e){console.error('Gemini AI error:',e);const msg=String(e?.message||e||'Gemini request failed');if(/GEMINI_REQUEST_TIMEOUT/i.test(msg)){toast('Gemini request timed out after 30 seconds. Check your internet connection and Firebase AI Logic status.')}else if(/app.?check|permission-denied|403|unauthorized/i.test(msg)){toast('Gemini request was blocked by Firebase App Check. Verify the registered domain matches the app URL and AI Logic App Check is enforced.')}else if(/not found|404|model/i.test(msg)){toast(`Gemini model error: ${msg}. Try Gemini 3.8 Flash.`)}else{toast(`Gemini error: ${msg}`)}return ''}}
